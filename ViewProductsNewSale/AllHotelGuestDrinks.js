...
const AllHotelGuestDrinks =({route, navigation}) => {
...
console.log("HEADER NAME",HeaderName);
 ...
 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostHotelDrinksProducts/');
...
// State variable to store the RoomClasses data
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostHotelDrinksCategories/`)
 ...
 const handleSubmit = () => {
  if (
    ...
    fetch(EndPoint + '/PostData/PostHotelDrinksProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Hotel/HotelDrinksProducts/?id=${id}&page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
const addCartItem = async () => {
  if (!selectedProduct || !quantity || !selectedTable || !selectedRoom) {
...
//navigation.replace('All Hotel Guest Drinks',id);
navigation.replace('All Hotel Guest Drinks', { id });
  ...
  try {
    const response = await axios.post(
     ...
     //setShouldReload(true); 
      //navigation.replace('All Hotel Guest Drinks'); 
...
const removeCartItem = (cartId) => {
  ...
  const apiUrl = `${EndPoint}/Cart/HotelDrinksDeleteCartItem/?cartId=${cartId}`;
...
const makeOrder = async () => {
  ...
  try {
    const response = await axios.post(
      EndPoint + '/Cart/HotelDrinksOrder/',
...
<AddMinorHeader title={HeaderName} pressMe={AddDrinks} screenName="Hotel Drinks Guest Customers" />
          ...
          <View style={globalstyles.ModalView}>
            ...
            <View style={globalstyles.form}>
               ...
               <View style={globalstyles.ButtonConatiner}>
                    ...
                    <TouchableOpacity 
                    style={globalstyles.ButtonAdd}  
                    onPress={() => navigation.navigate('Drinks Hotel Waiters')} >
                        <Text style={{
                          color:'white',
                        }}>CONFIRM</Text>
                    </TouchableOpacity>
...
export default AllHotelGuestDrinks;

...
const RetailsDrinksProducts =({route, navigation}) => {
...
console.log("HEADER NAME",HeaderName);
 ...
 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRetailsDrinksProducts/');
...
// State variable to store the RoomClasses data
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRetailsDrinksCategories/`)
 ...
 const handleSubmit = () => {
  if (
    ...
    fetch(EndPoint + '/PostData/PostRetailsDrinksProducts/', {
      method: 'POST',
      headers: {
...
const AddDrinks =() =>{
 ...
 //    DrinksImage:require('../assets/i2.png'),
 //    price:15000,
 //    id:'1'
 //  },
 ...
 //    Unit:'Chupa 1 1',
 //    product_second_name:'Maharage',
 //    DrinksImage:require('../assets/i3.png'),
 //    price:1000,
 //    id:'2'
 //  },
 ...
 //    DrinksImage:require('../assets/i4.png'),
 //    price:4000,
 //    id:'3'
 //  },
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Retails/RetailsDrinksProducts/?id=${id}&page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
const addCartItem = async () => {
  if (!selectedProduct || !quantity || !selectedTable) {
...
//navigation.replace('Retails Drinks Products',id);
navigation.replace('Retails Drinks Products', { id });
  ...
  try {
    const response = await axios.post(
      EndPoint + '/Cart/RetailsDrinksAddToCartWithoutRoom/',
      {
     ...
     //setShouldReload(true); 
      //navigation.replace('Retails Drinks Products'); 
...
const fetchCartItems = async () => {
  try {
    const response = await axios.get(
      EndPoint + '/Cart/RetailsDrinksCart/',
      {
        headers: {
          Authorization: `Token ${userToken}`,
        },
...
const removeCartItem = (cartId) => {
  ...
  const apiUrl = `${EndPoint}/Cart/RetailsDrinksDeleteCartItem/?cartId=${cartId}`;
...
const makeOrder = async () => {
  ...
  try {
    const response = await axios.post(
      EndPoint + '/Cart/RetailsDrinksOrder/',
...
<AddMinorHeader title={HeaderName} pressMe={AddDrinks} screenName="Retails NewSale Drinks" />
...
export default RetailsDrinksProducts;

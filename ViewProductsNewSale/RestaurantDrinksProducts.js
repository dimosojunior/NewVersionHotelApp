...
const RestaurantDrinksProducts =({route, navigation}) => {
...
console.log("HEADER NAME",HeaderName);
 ...
 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRestaurantDrinksProducts/');
...
// State variable to store the RoomClasses data
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRestaurantDrinksCategories/`)
 ...
 const handleSubmit = () => {
  if (
    ...
    fetch(EndPoint + '/PostData/PostRestaurantDrinksProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Restaurant/RestaurantDrinksProducts/?id=${id}&page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
//navigation.replace('Restaurant Drinks Products',id);
navigation.replace('Restaurant Drinks Products', { id });
  ...
  try {
    const response = await axios.post(
      EndPoint + '/Cart/RestaurantDrinksAddToCartWithoutRoom/',
      {
     ...
     //setShouldReload(true); 
      //navigation.replace('Restaurant Drinks Products'); 
...
<AddMinorHeader title={HeaderName} pressMe={AddDrinks} screenName="Restaurant NewSale Drinks" />
          ...
          <View style={globalstyles.ModalView}>
            ...
            <View style={globalstyles.form}>
               ...
               <View style={globalstyles.ButtonConatiner}>
                    ...
                    <TouchableOpacity 
                    style={globalstyles.ButtonAdd}  
                    onPress={() => navigation.navigate('Drinks Restaurant Waiters')} >
                        <Text style={{
                          color:'white',
                        }}>CONFIRM</Text>
                    </TouchableOpacity>
...
export default RestaurantDrinksProducts;

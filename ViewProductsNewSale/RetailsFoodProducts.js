...
const RetailsFoodProducts =({route, navigation}) => {
...
console.log("HEADER NAME",HeaderName);
 ...
 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRetailsFoodProducts/');
...
// State variable to store the RoomClasses data
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRetailsFoodCategories/`)
 ...
 const handleSubmit = () => {
  if (
    ...
    fetch(EndPoint + '/PostData/PostRetailsFoodProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Retails/RetailsFoodProducts/?id=${id}&page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
//navigation.replace('Retails Food Products',id);
navigation.replace('Retails Food Products', { id });
  ...
  try {
    const response = await axios.post(
      EndPoint + '/Cart/RetailsFoodAddToCartWithoutRoom/',
      {
     ...
     //setShouldReload(true); 
      //navigation.replace('Retails Food Products'); 
...
const removeCartItem = (cartId) => {
  ...
  const apiUrl = `${EndPoint}/Cart/RetailsFoodDeleteCartItem/?cartId=${cartId}`;
...
const makeOrder = async () => {
  ...
  try {
    const response = await axios.post(
      EndPoint + '/Cart/RetailsFoodOrder/',
...
<AddMinorHeader title={HeaderName} pressMe={AddFood} screenName="Retails NewSale Food" />
...
export default RetailsFoodProducts;

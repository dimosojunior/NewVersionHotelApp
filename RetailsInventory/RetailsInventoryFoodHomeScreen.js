...
const RetailsInventoryFoodHomeScreen =({navigation }) => {
...
const AddFood =() =>{
 ...
 //FOR APIS
const { datas:inventory,setDatas:setInventory, isPending, error } = useFetch(EndPoint+'/PostData/PostRetailsFoodCategories/');
...
<MinorHeader title="Food" pressMe={AddFood} screenName="Retails Inventory" />
...
{inventory && inventory.length > 0 ? (
      ...
      <FlatList
        ...
        renderItem={({item, index}) => {
          ...
          <TouchableOpacity 
             activeOpacity={1}
             onPress={() => navigation.navigate('Retails Inventory Food Products', item)}
            >
...
export default RetailsInventoryFoodHomeScreen;

...
const RetailsNewSaleSoftDrinks =({navigation}) => {

 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRetailsSoftDrinksProducts/');
 ...
 const handleSubmit = () => {
  if (
    ...
    fetch(EndPoint + '/PostData/PostRetailsSoftDrinksProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Retails/RetailsSoftDrinksProducts/?page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
navigation.replace('Retails NewSale Soft Drinks'); 
...
<AddMinorHeader title="Soft Drinks" pressMe={AddFood} screenName="Retails NewSale Drinks" />
...
export default RetailsNewSaleSoftDrinks;

...
const RetailsNewSaleOtherFood =({navigation}) => {

 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRetailsOtherFoodProducts/');
 ...
 const handleSubmit = () => {
  if (
    ...
    fetch(EndPoint + '/PostData/PostRetailsOtherFoodProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Retails/RetailsOtherFoodProducts/?page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
navigation.replace('Retails NewSale Other Food'); 
...
<AddMinorHeader title="Food" pressMe={AddFood} screenName="Retails NewSale Food" />
...
export default RetailsNewSaleOtherFood;

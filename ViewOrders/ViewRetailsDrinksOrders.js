...
const ViewRetailsDrinksOrders = ({route ,navigation }) => {
...
const [isRange, setisRange] = useState(false);
  ...
  useEffect(() => {
    ...
    axios.get(`${EndPoint}/Cart/GetRetailsDrinksOrderItems/?id=${id}`)
...
const removeOrderItem = (cartId) => {
  console.log("CARTID",cartId);
   ...
   // navigation.replace('View Retails Drinks Orders', {id}); 
   navigation.replace('Drinks Retails Waiters');
  // Make an API request to delete the item from the cart
  const apiUrl = `${EndPoint}/Cart/RetailsDrinksDeleteOrderItem/?cartId=${cartId}`;
    ...
    .then((response) => {
      if (response.status === 204) {
        ...
        setQueryset(queryset.filter((item) => item.id !== cartId));
         ...
         // navigation.replace('Retails NewSale Other Drinks');  
...
const setOrderStatusTrue = () => {
  ...
  const url = `${EndPoint}/Cart/RetailsDrinksOrderChangeStatusToTrue/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
const setOrderStatusFalse = () => {
  navigation.replace('Drinks Retails Waiters');
  setModalVisibleProcessOrder(false);
  const url = `${EndPoint}/Cart/RetailsDrinksOrderChangeStatusToFalse/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
<MinorHeader title="Items"  screenName="Drinks Retails Waiters" />
...
export default ViewRetailsDrinksOrders;

...
const ViewRestaurantFoodOrders = ({route ,navigation }) => {
...
const [isRange, setisRange] = useState(false);
  ...
  useEffect(() => {
    ...
    axios.get(`${EndPoint}/Cart/GetRestaurantFoodOrderItems/?id=${id}`)
...
const removeOrderItem = (cartId) => {
  console.log("CARTID",cartId);
   ...
   // navigation.replace('View Restaurant Food Orders', {id}); 
   navigation.replace('Food Restaurant Waiters');
  // Make an API request to delete the item from the cart
  const apiUrl = `${EndPoint}/Cart/RestaurantFoodDeleteOrderItem/?cartId=${cartId}`;
...
const setOrderStatusTrue = () => {
  ...
  const url = `${EndPoint}/Cart/RestaurantFoodOrderChangeStatusToTrue/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
const setOrderStatusFalse = () => {
  navigation.replace('Food Restaurant Waiters');
  setModalVisibleProcessOrder(false);
  const url = `${EndPoint}/Cart/RestaurantFoodOrderChangeStatusToFalse/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
<MinorHeader title="Items"  screenName="Food Restaurant Waiters" />
...
export default ViewRestaurantFoodOrders;

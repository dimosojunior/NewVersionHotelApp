...
const ViewRetailsFoodOrders = ({route ,navigation }) => {
...
const [isRange, setisRange] = useState(false);
  ...
  useEffect(() => {
    ...
    axios.get(`${EndPoint}/Cart/GetRetailsFoodOrderItems/?id=${id}`)
...
const removeOrderItem = (cartId) => {
  console.log("CARTID",cartId);
   ...
   // navigation.replace('View Retails Food Orders', {id}); 
   navigation.replace('Food Retails Waiters');
  // Make an API request to delete the item from the cart
  const apiUrl = `${EndPoint}/Cart/RetailsFoodDeleteOrderItem/?cartId=${cartId}`;
...
const setOrderStatusTrue = () => {
  ...
  const url = `${EndPoint}/Cart/RetailsFoodOrderChangeStatusToTrue/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
const setOrderStatusFalse = () => {
  navigation.replace('Food Retails Waiters');
  setModalVisibleProcessOrder(false);
  const url = `${EndPoint}/Cart/RetailsFoodOrderChangeStatusToFalse/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
<MinorHeader title="Items"  screenName="Food Retails Waiters" />
...
export default ViewRetailsFoodOrders;

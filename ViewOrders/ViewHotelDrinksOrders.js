...
const ViewHotelDrinksOrders = ({route ,navigation }) => {
...
const [isRange, setisRange] = useState(false);
  ...
  useEffect(() => {
    ...
    axios.get(`${EndPoint}/Cart/GetHotelDrinksOrderItems/?id=${id}`)
...
const removeOrderItem = (cartId) => {
  console.log("CARTID",cartId);
   ...
   // navigation.replace('View Hotel Drinks Orders', {id}); 
   navigation.replace('Drinks Hotel Waiters');
  // Make an API request to delete the item from the cart
  const apiUrl = `${EndPoint}/Cart/HotelDrinksDeleteOrderItem/?cartId=${cartId}`;
    ...
    .then((response) => {
      if (response.status === 204) {
        ...
        setQueryset(queryset.filter((item) => item.id !== cartId));
         ...
         // navigation.replace('Hotel NewSale Other Drinks');  
...
const setOrderStatusTrue = () => {
  ...
  const url = `${EndPoint}/Cart/HotelDrinksOrderChangeStatusToTrue/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
const setOrderStatusFalse = () => {
  navigation.replace('Drinks Hotel Waiters');
  setModalVisibleProcessOrder(false);
  const url = `${EndPoint}/Cart/HotelDrinksOrderChangeStatusToFalse/?id=${id}`;
  fetch(url, {
    method: 'POST',
  })
    .then((response) => {
...
<MinorHeader title="Items"  screenName="Drinks Hotel Waiters" />
...
export default ViewHotelDrinksOrders;

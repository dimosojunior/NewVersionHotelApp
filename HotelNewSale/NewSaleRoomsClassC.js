...
const NewSaleRoomsClassC =({navigation}) => {
...
// ForeignKey Field
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRoomsClasses/`)
      ...
      .then((data) => {
        ...
        //setSelectedRoomClass(data[0]); // For example, set the first RoomClass Cs default
      })
      .catch((error) => {
 ...
 //FOR APIS
const { datas:myItems,setDatas:setMyItems, isPending, error } = useFetch(EndPoint+'/PostData/PostHotelRoomsClassC/');
...
const handleSubmit = () => {
    if (
      ...
      fetch(`${EndPoint}/PostData/PostHotelRoomsClassC/`, {
        method: 'POST',
        headers: {
...
navigation.replace('NewSale Rooms Class C'); 
...
const makeOrder = async () => {
  try {
    ...
    const updatedRoomsResponse = await axios.get(
      EndPoint + '/PostData/PostHotelRoomsClassC/'
    );
...
<AddMinorHeader title="Class C Rooms" pressMe={AddRoom} screenName="Hotel NewSale Rooms" />
...
export default NewSaleRoomsClassC;

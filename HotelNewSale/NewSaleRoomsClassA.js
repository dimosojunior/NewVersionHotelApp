...
const NewSaleRoomsClassA =({navigation}) => {
...
const handleSubmit = () => {
    if (
      ...
      fetch(`${EndPoint}/PostData/PostHotelRoomsClassA/`, {
        method: 'POST',
        headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Hotel/HotelRoomsClassA/?page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
navigation.replace('NewSale Rooms Class A'); 
...
const makeOrder = async () => {
  try {
    ...
    const updatedRoomsResponse = await axios.get(
      EndPoint + '/PostData/PostHotelRoomsClassA/'
    );
...
<AddMinorHeader title="Class A Rooms" pressMe={AddRoom} screenName="Hotel NewSale Rooms" />
...
export default NewSaleRoomsClassA;

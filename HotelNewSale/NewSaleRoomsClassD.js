...
const NewSaleRoomsClassD =({navigation}) => {
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRoomsClasses/`)
      ...
      .then((data) => {
        ...
        //setSelectedRoomClass(data[0]); // For example, set the first RoomClass Ds default
      })
      .catch((error) => {
...
navigation.replace('NewSale Rooms Class D'); 
...
const makeOrder = async () => {
  try {
    ...
    const updatedRoomsResponse = await axios.get(
      EndPoint + '/PostData/PostHotelRoomsClassD/'
    );
...
<AddMinorHeader title="Class D Rooms" pressMe={AddRoom} screenName="Hotel NewSale Rooms" />
...
export default NewSaleRoomsClassD;

...
const NewSaleRoomsClassB =({navigation}) => {
...
// ForeignKey Field
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRoomsClasses/`)
      ...
      .then((data) => {
        ...
        //setSelectedRoomClass(data[0]); // For example, set the first RoomClass Bs default
      })
      .catch((error) => {
 ...
 //FOR APIS
const { datas:myItems,setDatas:setMyItems, isPending, error } = useFetch(EndPoint+'/PostData/PostHotelRoomsClassB/');
...
const handleSubmit = () => {
    if (
      ...
      fetch(`${EndPoint}/PostData/PostHotelRoomsClassB/`, {
        method: 'POST',
        headers: {
...
navigation.replace('NewSale Rooms Class B'); 
  ...
  try {
    ...
    const updatedRooms = myItems.map((item) => {
...
const makeOrder = async () => {
  try {
    ...
    const updatedRoomsResponse = await axios.get(
      EndPoint + '/PostData/PostHotelRoomsClassB/'
    );

    // Set the updated list of rooms in your state (myItems)
    setMyItems(updatedRoomsResponse.data);
...
<AddMinorHeader title="Class B Rooms" pressMe={AddRoom} screenName="Hotel NewSale Rooms" />
...
export default NewSaleRoomsClassB;

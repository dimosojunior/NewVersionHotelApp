...
const NewSaleRoomsClassE =({navigation}) => {
...
// ForeignKey Field
  ...
  useEffect(() => {
    fetch(`${EndPoint}/PostData/PostRoomsClasses/`)
      ...
      .then((data) => {
        ...
        //setSelectedRoomClass(data[0]); // For example, set the first RoomClass Es default
      })
      .catch((error) => {
 ...
 //FOR APIS
const { datas:myItems,setDatas:setMyItems, isPending, error } = useFetch(EndPoint+'/PostData/PostHotelRoomsClassE/');
...
const handleSubmit = () => {
    if (
      ...
      fetch(`${EndPoint}/PostData/PostHotelRoomsClassE/`, {
        method: 'POST',
        headers: {
...
navigation.replace('NewSale Rooms Class E'); 
...
const makeOrder = async () => {
  try {
    ...
    const updatedRoomsResponse = await axios.get(
      EndPoint + '/PostData/PostHotelRoomsClassE/'
    );
...
<AddMinorHeader title="Class E Rooms" pressMe={AddRoom} screenName="Hotel NewSale Rooms" />
...
export default NewSaleRoomsClassE;

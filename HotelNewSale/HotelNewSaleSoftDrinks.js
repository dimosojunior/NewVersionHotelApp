...
const HotelNewSaleSoftDrinks =({navigation}) => {

 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostHotelSoftDrinksProducts/');
 ...
 const handleSubmit = () => {
  if (
    ...
    fetch(EndPoint + '/PostData/PostHotelSoftDrinksProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Hotel/HotelSoftDrinksProducts/?page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
navigation.replace('Hotel NewSale Soft Drinks'); 
...
<AddMinorHeader title="Soft Drinks" pressMe={AddFood} screenName="Hotel NewSale Drinks" />
...
export default HotelNewSaleSoftDrinks;

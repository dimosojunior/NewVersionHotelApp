...
const HotelNewSaleBeer =({navigation}) => {

 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostHotelBeersProducts/');
 ...
 const handleSubmit = () => {
  if (
    ...
    fetch(EndPoint + '/PostData/PostHotelBeersProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Hotel/HotelBeersProducts/?page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
navigation.replace('Hotel NewSale Beers'); 
...
<AddMinorHeader title="Beers" pressMe={AddFood} screenName="Hotel NewSale Drinks" />
...
export default HotelNewSaleBeer;

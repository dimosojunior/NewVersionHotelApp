...
import { View, Text, FlatList,TouchableOpacity, ScrollView, Image, StyleSheet, ActivityIndicator, StatusBar } from "react-native";
...
const BusinessUnitScreen = () => {
  ...
  const getUsers = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Hotel/HotelBusinessUnit/?page=${current_page}&page_size=5` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
  ...
  const RenderItem = ({ item }) => {
    return (
      <View key={item.id} style={styles.itemWrapperStyle}>
       ...
       <TouchableOpacity 
      onPress={() => navigation.navigate('View Business Unit', item)}>

     <View style={styles.contentWrapperStyle}>
     <Text style={styles.txtNameStyle}>{`${item.id} `}</Text>

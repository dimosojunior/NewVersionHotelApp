...
const RestaurantNewSaleSoftDrinks =({navigation}) => {

 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRestaurantSoftDrinksProducts/');
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Restaurant/RestaurantSoftDrinksProducts/?page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
navigation.replace('Restaurant NewSale Soft Drinks'); 
...
<AddMinorHeader title="Soft Drinks" pressMe={AddFood} screenName="Restaurant NewSale Drinks" />
...
export default RestaurantNewSaleSoftDrinks;

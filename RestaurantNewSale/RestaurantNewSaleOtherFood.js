...
const RestaurantNewSaleOtherFood =({navigation}) => {

 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRestaurantOtherFoodProducts/');
 ...
 const handleSubmit = () => {
  if (
    ...
    const formData = {
      ...product,
      productCategory: 'Other Food', // Set the value of ProductCategory here
    };

    fetch(EndPoint + '/PostData/PostRestaurantOtherFoodProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Restaurant/RestaurantOtherFoodProducts/?page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
navigation.replace('Restaurant NewSale Other Food'); 
  ...
  try {
    const response = await axios.post(
      EndPoint + '/Cart/RestaurantFoodCart/',
      {
...
const removeCartItem = (cartId) => {
  ...
  const apiUrl = `${EndPoint}/Cart/RestaurantFoodDeleteCartItem/?cartId=${cartId}`;
...
const makeOrder = async () => {
  ...
  try {
    const response = await axios.post(
      EndPoint + '/Cart/RestaurantFoodOrder/',
...
<AddMinorHeader title="Food" pressMe={AddFood} screenName="Restaurant NewSale Food" />
...
export default RestaurantNewSaleOtherFood;

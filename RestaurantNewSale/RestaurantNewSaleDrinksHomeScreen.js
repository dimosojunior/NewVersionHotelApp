...
const RestaurantNewSaleDrinksHomeScreen =({navigation }) => {
...
const AddFood =() =>{
 ...
 //FOR APIS
const { datas:inventory,setDatas:setInventory, isPending, error } = useFetch(EndPoint+'/PostData/PostRestaurantDrinksCategories/');
...
const [CategoryName, setCategoryName] = useState('');
    const move = (CategoryName) =>{
      ...
      navigation.navigate('Restaurant NewSale ' +CategoryName);
    }
...
<MinorHeader title="Drinks" pressMe={AddFood} screenName="Restaurant NewSale" />
        ...
        renderItem={({item, index}) => {
          ...
          return (
           <>
            <TouchableOpacity 
             activeOpacity={1}
             onPress={() => navigation.navigate('Restaurant Drinks Products', item)}
            >
...
export default RestaurantNewSaleDrinksHomeScreen;

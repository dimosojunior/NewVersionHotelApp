...
const RestaurantNewSalePizza =({navigation}) => {

 //FOR APIS
//const { datas:myItems,setDatas:setQueryset, isLoa, error } = useFetch(EndPoint+'/PostData/PostRestaurantPizzaProducts/');
 ...
 const handleSubmit = () => {
  if (
    ...
    const formData = {
      ...product,
      productCategory: 'Pizza', // Set the value of ProductCategory here
    };

    fetch(EndPoint + '/PostData/PostRestaurantPizzaProducts/', {
      method: 'POST',
      headers: {
...
const [isPending, setPending] = useState(true);
  ...
  const getProducts = () => {
    if (endReached===true) {
      ...
      setIsLoading(true)
       const url = EndPoint+`/Restaurant/RestaurantPizzaProducts/?page=${current_page}&page_size=8` 
       console.log(url)
       fetch(url)
       .then(res=>res.json())
       .then(data=>{
...
<AddMinorHeader title="Pizza" pressMe={AddFood} screenName="Restaurant NewSale Food" />
...
{/*mwisho wa make order*/}
    ...
    <View style={{
      flex:1
               ...
               <View style={globalstyles.CartListHeaderContainer}>
                <Text style={globalstyles.CartListHeader}>Pizza</Text>
                </View>
...
export default RestaurantNewSalePizza;

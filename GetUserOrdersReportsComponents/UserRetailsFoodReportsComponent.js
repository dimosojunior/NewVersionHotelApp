...
const UserRetailsFoodReportsComponent = ({id, username}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/RetailsFoodOrder/?page=${current_page}&page_size=2`;
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterRetailsFoodOrderReport/?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
const FoodReportCard = ({item, index}) => {
    ...
    return(
     ...
     {item.total_price > 0 && (
        <Pressable 
          // onPress={() => navigation.navigate('View Retails Food Orders', item)}
...
export default UserRetailsFoodReportsComponent;

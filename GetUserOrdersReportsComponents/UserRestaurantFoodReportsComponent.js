...
const UserRestaurantFoodReportsComponent = ({id, username}) => {
...
const checkLoggedIn = async () => {
  const token = await AsyncStorage.getItem('userToken');
  setUserToken(token);
  console.log("USERTOKEN ARE", userToken)
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/RestaurantFoodOrder/?page=${current_page}&page_size=2`;
...
//mwisho wa load more
  ...
  const handleFilter = () => {
    ...
    axios
      .get(
        `${EndPoint}/Cart/FilterRestaurantFoodOrderReport/?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      )
      .then((response) => {
...
const FoodReportCard = ({item, index}) => {
    ...
    return(
     ...
     {item.total_price > 0 && (
        <Pressable 
          // onPress={() => navigation.navigate('View Restaurant Food Orders', item)}
...
export default UserRestaurantFoodReportsComponent;

...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/Cart/HotelFoodOrder/?page=${current_page}&page_size=2`;
...
const FoodReportCard = ({item, index}) => {
    ...
    return(
     ...
     <Text style={globalstyles.ReportScreentitle}>{item.id}</Text>
     </View>
...
{orders && item.user && item.user.username ? ( 
           <View style={globalstyles.ReportScreenItemscontainer2}>
...
{orders && orders.length > 0 ? (
 ...
 <View style={globalstyles.CartListHeaderContainer}>
  ...
  <Text style={globalstyles.CartListHeader}>TEST Report</Text>
  {isRange ? (
